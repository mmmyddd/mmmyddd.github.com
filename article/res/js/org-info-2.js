...
}else{this.cutToc(g,f);}}}}}},mkNodeFromHref:function(r){if(r.match(this.REGEX)){var g=this.REGEX.exec(r);
...
this.ST.push(o);}this.TAGS[o].push(l);}}else{if(m[e].className.match(this.SECNUMX)){this.SN_MAP[this.trim(m[e].innerHTML)]=this.N;
...
b.R.CH[a].fold();}}}if("?/toc/?"!=d&&null!=b.TL){b.TL.blur();}if("?/toc/?"==d||(!isNaN(e)&&b.S[e])){if("?/toc/?"==d&&b.HIDE_TOC){b.N=b.TOC;
...
}else{b.N.D.scrollIntoView(true);}}}}},pV:function(d,a){var c=this;document.onclick=null;
document.ondblclick=null;c.VIEW=c.PLAIN_VIEW;OrgNode.hE(c.W);if(c.IT){OrgNode.hE(c.IT);
}OrgNode.sE(c.T);if(c.W.firstChild){c.W.removeChild(c.W.firstChild);}c.R.sAC();for(var b=0;
b<c.R.CH.length;++b){c.R.CH[b].ST=OrgNode.SU;c.R.CH[b].fold();}if(!a){c.ss(d);}if(c.P){OrgNode.sE(c.P);
}if(c.N.J==0){window.scrollTo(0,0);}else{c.N.D.scrollIntoView(true);}},iV:function(c,a){var b=this;
document.onclick=null;document.ondblclick=null;b.VIEW=b.INFO_VIEW;b.unhH(b.N.J);if(b.IT&&!b.FIXED_TOC){OrgNode.sE(b.IT);
OrgNode.hE(b.T);}OrgNode.sE(b.W);b.R.hAC();if(b.TOC&&!b.FIXED_TOC){OrgNode.hE(b.TOC.D);
}if(b.P){OrgNode.sE(b.P);}if(!a){b.ss(c);}window.scrollTo(0,0);},sV:function(c,a){var b=this;
b.VIEW=b.SLIDE_VIEW;b.unhH(b.N.J);OrgNode.hE(b.T);if(b.IT){OrgNode.hE(b.IT);}if(b.TOC){OrgNode.hE(b.TOC.D);
}OrgNode.sE(b.T);OrgNode.sE(b.W);b.R.hAC();OrgNode.hE(b.TOC.D);if(b.P){OrgNode.hE(b.P);
}b.adjustSlide(c);if(!a){b.ss(c);}},adjustSlide:function(f,g){var k=true;var e=true;
var d=false;if(f>this.N.J){d=true;}if(null==g){d=true;}if(d){for(var a=this.S[f].F.firstChild;
...
}}else{if("?"==c||"驴"==c){b.showHelp();}else{if("C"==c){if(b.ST.length){b.showTagsIndex();
